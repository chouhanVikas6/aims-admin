...
const AppSidebar: React.FC = () => {
    ...
    // Sync openSubmenu with pathname changes
    useEffect(() => {
                ...
                <li key={nav.name}>
                    {nav.subItems ? (
                        ...
                        >
                            ...
                            {(isExpanded || isHovered || isMobileOpen) && (
                                <ChevronDownIcon
                                    className={`ml-auto w-5 h-5 transition-transform duration-200 ${openSubmenu?.type === "main" && openSubmenu?.index === index
        ...
        >
            ...
            >
                <Link href="/" className="flex items-center gap-2">
                    {isExpanded || isHovered || isMobileOpen ? (
                        <span className="text-2xl font-bold text-brand-500">AIMS Admin</span>
                    ) : (
                        <span className="text-2xl font-bold text-brand-500">A</span>
                    )}
                </Link>
            </div>
